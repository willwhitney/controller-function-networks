...
if string.len(seed_text) > 0 then
    ...
    for c in seed_text:gmatch'.' do
        ...
        local input = one_hot:forward(prev_char)
        prediction = model:step(input)
    end
else
    -- fill with uniform probabilities over characters (? hmm)
    gprint('missing seed text, using uniform probability over first character')
    gprint('--------------------------')
    local char = torch.Tensor{torch.rand(1):mul(#ivocab):round():add(1)[1]}:double()
    -- io.write(ivocab[char[1]])
    local input = one_hot:forward(char):double()
...
for i=1, opt.length do
    ...
    if opt.sample == 0 then
        ...
        prev_char = torch.multinomial(probs:double(), 1):resize(1):double()
    end
