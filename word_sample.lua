...
for i = 1, 10 do
    ...
    if string.len(seed_text) > 0 then
        ...
        gprint('--------------------------')
        for word in seed_text:gmatch("%w+") do
            word_tensor = torch.Tensor{vocab[word]}
            io.write(word, ' ')
            -- io.write(ivocab[prev_char[1]])
            -- if opt.gpuid >= 0 and opt.opencl == 0 then prev_char = prev_char:cuda() end
            -- if opt.gpuid >= 0 and opt.opencl == 1 then prev_char = prev_char:cl() end
            local input = one_hot:forward(word_tensor)
            prediction = recurrent:step(input)
        end
    else
        -- fill with uniform probabilities over characters (? hmm)
        -- gprint('missing seed text, using uniform probability over first character')
        gprint('--------------------------')
        local word = torch.Tensor{torch.rand(1):mul(10000):round():add(1)[1]}:double()

        io.write(ivocab[word[1]], ' ')
        local input = one_hot:forward(word):double()
    ...
    for i=1, opt.length do
        ...
        if opt.sample == 0 then
            ...
            -- use sampling
            -- prediction = softmax:forward(prediction)
            prediction:div(opt.temperature) -- scale by temperature
            local probs = torch.exp(prediction):squeeze()
            probs:div(torch.sum(probs)) -- renormalize so probs sum to one
            prev_word = torch.multinomial(probs:double(), 1):resize(1):double()
        end

        -- forward the rnn for next character
        local input = one_hot:forward(prev_word)
        prediction = recurrent:step(input)
        -- print(vis.simplestr(model.controller.output[1]))
