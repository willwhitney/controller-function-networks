...
function CFNetwork:__init(options)
    ...
    for i = 1, options.num_functions do
        ...
        --     layer:add(nn.Linear(options.encoded_dimension, options.encoded_dimension))
        --     layer:add(nn.PReLU())
        -- elseif options.function_nonlinearity == 'lstm' then
            local layer = nn.SteppableLSTM(options.encoded_dimension, options.encoded_dimension, options.encoded_dimension, 1, 0)
            -- layer:add(nn.PReLU())
...
function CFNetwork:step(input)
    ...
    for substep = 1, self.steps_per_output do
        ...
        for i = 1, #self.functions do
            local function_output = self.functions[i]:step(next_input):clone()
...
function CFNetwork:backstep(input, gradOutput)
    ...
    for substep = self.steps_per_output, 1, -1 do
        ...
        local function_outputs = {}
        for i = 1, #self.functions do
            local function_step_trace = self.functions[i].trace[#self.functions[i].trace]
            local function_output = function_step_trace[#function_step_trace].output
            table.insert(function_outputs, function_output)
        end
        ...
        for i = 1, #self.functions do
            current_gradInput = current_gradInput + self.functions[i]:backstep(substep_input, grad_function_outputs[i])
        end
    end
